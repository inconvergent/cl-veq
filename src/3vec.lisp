...
(defun 3len2 (a)
  (declare #.*opt-settings* (3vec a))
  (+ (mul2 (3vec-x a)) (mul2 (3vec-y a)) (mul2 (3vec-z a))))

(declaim (inline 3len))
(defun 3len (a)
  (declare #.*opt-settings* (3vec a))
  (sqrt (the pos-double (3len2 a))))
...
(defun 3dst2 (a b)
  (declare #.*opt-settings* (3vec a b))
  (+ (mul2 (- (3vec-x a) (3vec-x b)))
     (mul2 (- (3vec-y a) (3vec-y b)))
     (mul2 (- (3vec-z a) (3vec-z b)))))

(declaim (inline 3dst))
(defun 3dst (a b)
  (declare #.*opt-settings* (3vec a b))
  (sqrt (the pos-double (3dst2 a b))))
...
(declaim (inline 3pnorm))
(defun 3pnorm (v p)
  (declare #.*opt-settings* (vec:3vec v) (double-float p))
  (expt (+ (expt (abs (vec:3vec-x v)) p)
           (expt (abs (vec:3vec-y v)) p)
           (expt (abs (vec:3vec-z v)) p))
        (/ p)))
