  ...
  (labels ((num-types (o)
             (cond ((numberp o) (coerce o type))
                   ...
                   ((consp o) (cons (num-types (car o))
                                    (num-types (cdr o)))))))
    (let ((x* (car args))
          (in (vvsym type 1 (symb :ease-in- name))) ; fease-in-linear
          (out (vvsym type 1 (symb :ease-out- name)))
          (inout (vvsym type 1 (symb :ease-in-out- name))))
      (num-types
        `(fvprogn (export ',in) (export ',out) (export ',inout)
                  (defun ,in ,args
                    ,(format nil "ease in:~%arg: ~a~%body: ~a" args (car body))
                    (let ((,x* (,(vvsym type 1 :clamp) ,x*))) ,@body))
                  (defun ,out ,args
                    ,(format nil "ease out:~%arg: ~a~%body: ~a" args (car body))
                    (let ((,x* (,(vvsym type 1 :clamp) (- 1 ,x*))))
                      (1+ (- ,@body))))
                  (defun ,inout ,args
                    ,(format nil "ease in-out:~%arg: ~a~%body: ~a" args (car body))
                    (let ((,x* (,(vvsym type 1 :clamp) ,x*)))
